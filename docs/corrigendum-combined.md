...
| 6 | Sent 2026-09-13 from `bkennedy1@captechu.edu` to `bookproofing@igi-global.com`, cc the volume editor and `cust@igi-global.com`, with the corrected Appendix B attached. The text as sent is in `corrigendum-sent-2026-09-13.md`: same content as this letter, with dashes removed and set for a plain-text email. The proofing desk auto-replied that the inbox is loosely monitored and that mail without a matching note in the proofing system may not reach the typesetter; a second route may be needed. | sent |
| 7 | Run `notebooks/stac_v2_colab.ipynb` top to bottom on `main` once, since Part IV offers it to the editor as a verification path. Done 2026-09-07 on CPU, every section (log in `corrigendum-2026-07.md`). It surfaced one failing test in §3 and one stale warning in §7, both fixed and in the changelog. A GPU run of §8 at the notebook's default 300 steps remains optional. | done |
| 8 | Return the corrections as comments inside the published PDF, which is what the proofing desk asked for on 2026-09-14. Done the same day: `Aligned-Minds-Efficient-Machines-CORRECTIONS-MARKED.pdf`, 15 numbered highlight comments anchored on the passages they concern plus a summary note on printed p. 113. Reading the typeset text changed three things, all recorded in [`corrigendum-marked-copy-2026-09-14.md`](corrigendum-marked-copy-2026-09-14.md). | done |

Send only when every row reads done. Rows 1 to 7 governed the letter sent on 2026-09-13; row 8 covers the marked copy the publisher asked for in reply. The send dates are recorded in
[`corrigendum-2026-07.md`](corrigendum-2026-07.md).
...
### Ostrau et al. (2022) — withdrawn 2026-09-14

**Issue as reported.** Cited in the text six times, including in the Abstract, but absent from
the reference list of the submitted manuscript.

**Resolved, no action.** The typeset chapter carries the entry on printed p. 142, in
alphabetical position, exactly as Crossref's 2026-08-27 deposit indicated. Production added it.
The item was already worded conditionally in the letter sent on 2026-09-13; it is withdrawn
outright in the marked copy returned on 2026-09-14, with an apology for the confusion.
