...
| 3 | Diff every **Published** passage in Parts I and II against the chapter text. Done 2026-09-07 against the submitted manuscript with appendices (Drive: `…Submission_With_Appendices_Edit12.pdf`, 2025-10-12): all five Part I passages match verbatim; the Appendix B labels, ranges, and Global Note match; the reference-list defects (truncated `140532`, shared `15867066`, Ostrau et al. cited six times with no entry) are all present in the manuscript. Two labels in this file were abbreviated and are now quoted exactly. Remaining: a final read against the **typeset** chapter PDF, which lives on IGI's proofing platform rather than in Drive. Two things to check there specifically: house-style copyedits, and the Appendix B table column alignment — text extraction interleaves that table's columns, so the 6–12 Impulsivity row's label was read from row order rather than directly, and Correction ② rests on it. | done against manuscript; typeset check pending |
